...
class AuthenticationRepositoryImpl extends GetxController
  ...
  void _setInitialScreen(User? user) async {
    if (user == null) {
      ...
      fcmToken.value = storedFcmToken == '' ? '' : storedFcmToken;
      id.value = storedUid == '' ? user.uid : storedUid;
      ...
      try {
        ...
        // // Update the user's fcm token in the database in firestore
        // if (fcmToken.value != '') {
        //   await firestore
        //       .collection('users')
        //       .doc(currentUser.value.userID)
        //       .update({'fcmToken': fcmToken.value}).then(
        //           (value) => print("FCM TOKEN UPDATED IN FIRESTORE"));
        // }

        // // Update the user's fcm token in the database in local storage
        // currentUser.value.fcmToken = fcmToken.value;

        // prefs.setString('uid', currentUser.value.userID);
    ...
    try {
      ...
      if (googleSignInAccount != null) {
        ...
        try {
          ...
          SharedPreferences prefs = await SharedPreferences.getInstance();
          prefs.setString('uid', firebaseUser.value!.uid);
          ...
          prefs.setString('fcm_token', fcmToken.value);
  ...
  Future<void> updateFcmToken(String token) async {
    print("INSIDE UPDATE FCM TOKEN");
    fcmToken.value = token;

    // Update the user's fcm token in the database in local and remote storage
    currentUser.value.fcmToken = token;

    print("Current User Fcm Token ${currentUser.value.fcmToken}");

    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString('fcm_token', token).then((value) {
      print("FCM TOKEN UPDATED IN SHARED PREFERENCES");
    });

    final userId = prefs.getString('uid') ?? '';

    // Update the user's fcm token in the database in remote storage only user is logged in
    try {
      print("Now updating the fcm token in the firestore");
      ...
      log("${e.toString()} FCM TOKEN UPDATE FAILED");
    }
  }
  ...
  void _updateUserDataFromSharedPreferences(SharedPreferences prefs) {
    ...
    fcmToken.value = prefs.getString('fcm_token') ?? '';
    id.value = prefs.getString('uid') ?? '';
  }
